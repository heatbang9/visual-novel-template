...
@export var Audio_Button_Path: NodePath = @"HBoxContainer/PlayAudioButton"
@export var Text_Label_Path: NodePath = @"HBoxContainer/RichTextLabel"
...
func load_theme(theme: ConfigFile):
	...
	TextLabel.set('theme_override_fonts/normal_font', theme_font)
	TextLabel.set('theme_override_fonts/bold_font', DialogicUtil.path_fixer_load(theme.get_value('text', 'bold_font', 'res://addons/dialogic/Example Assets/Fonts/DefaultBoldFont.tres')))
	TextLabel.set('theme_override_fonts/italics_font', DialogicUtil.path_fixer_load(theme.get_value('text', 'italic_font', 'res://addons/dialogic/Example Assets/Fonts/DefaultItalicFont.tres')))
	...
	if alignment <= 2: # top
		TextContainer.alignment = BoxContainer.ALIGNMENT_BEGIN
	elif alignment <= 5: # center
		TextContainer.alignment = BoxContainer.ALIGNMENT_CENTER
	elif alignment <= 8: # bottom
		TextContainer.alignment = BoxContainer.ALIGNMENT_END
	
	var text_color = Color(theme.get_value('text', 'color', '#ffffffff'))
	TextLabel.set('theme_override_colors/default_color', text_color)
	#name_label.set('custom_colors/font_color', text_color)

	TextLabel.set('theme_override_colors/font_shadow_color', Color('#00ffffff'))
	...
	if theme.get_value('text', 'shadow', false):
		...
		TextLabel.set('theme_override_colors/font_shadow_color', text_shadow_color)

	var shadow_offset = theme.get_value('text', 'shadow_offset', Vector2(2,2))
	TextLabel.set('theme_override_constants/shadow_offset_x', shadow_offset.x)
	TextLabel.set('theme_override_constants/shadow_offset_y', shadow_offset.y)

	# Margin
	var text_margin = theme.get_value('text', 'margin', Vector2(20, 10))
	TextContainer.set('offset_left', text_margin.x)
	TextContainer.set('offset_right', text_margin.x * -1)
	TextContainer.set('offset_top', text_margin.y)
	TextContainer.set('offset_bottom', text_margin.y * -1)
