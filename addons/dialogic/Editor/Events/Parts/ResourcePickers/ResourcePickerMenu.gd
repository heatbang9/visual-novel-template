...
@export var custom_icon: Texture2D: set = set_custom_icon
var custom_icon_modulation : set = set_custom_icon_modulation
var menu_background
...
func _ready():
	...
	add_theme_color_override("font_color", default_color) #get_color("mono_color", "Editor"))
	update_submenu_style(get_popup())
	reset_modulation()
	$Icon2.texture = get_icon("Collapse", "EditorIcons")

func update_submenu_style(submenu):
	submenu.add_theme_stylebox_override('panel', menu_background)
	submenu.add_theme_stylebox_override('hover', StyleBoxEmpty.new())
	submenu.add_theme_color_override('font_color_hover', get_color("accent_color", "Editor"))

func set_custom_icon(texture:Texture2D):
