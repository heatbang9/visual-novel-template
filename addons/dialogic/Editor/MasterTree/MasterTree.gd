...
func _on_TimelineRootPopupMenu_id_pressed(id):
	...
	if id == 1: # add subfolder
		DialogicUtil.add_folder(editor_reference.flat_structure, "Timelines", get_selected().get_metadata(0), "New Folder "+str(Time.get_unix_time_from_system()))
		build_flat_tree_items("Timelines")
...
func _on_CharacterRootPopupMenu_id_pressed(id):
	if id == 0: # Add Character
		new_character()
	if id == 1: # add subfolder
		DialogicUtil.add_folder(editor_reference.flat_structure, "Characters", get_selected().get_metadata(0), "New Folder "+str(Time.get_unix_time_from_system()))
...
func _on_DefinitionRootPopupMenu_id_pressed(id):
	...
	if id == 2: # add subfolder
		DialogicUtil.add_folder(editor_reference.flat_structure, "Definitions", get_selected().get_metadata(0), "New Folder "+str(Time.get_unix_time_from_system()))
		build_flat_tree_items("Definitions")
...
func _on_ThemeRootPopupMenu_id_pressed(id):
	if id == 0: # Add Theme
		new_theme()
	if id == 1: # add subfolder
		DialogicUtil.add_folder(editor_reference.flat_structure, "Themes", get_selected().get_metadata(0), "New Folder "+str(Time.get_unix_time_from_system()))
		build_flat_tree_items("Themes")
	if id == 2: # remove folder and substuff
		if get_selected().get_parent() == get_root():
			return
