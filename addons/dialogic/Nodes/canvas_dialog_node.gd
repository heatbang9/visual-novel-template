...
func set_dialog_node_scene(scene) -> void:
	...
	if dialog_node:
		...
		assert(_err == OK)
		_err = dialog_node.connect("timeline_changed", Callable(self, "_on_timeline_changed"))
		assert(_err == OK)
		...
		assert(_err == OK)
		_err = dialog_node.connect("auto_advance_toggled", Callable(self, "_on_auto_advance_toggle"))
		assert(_err == OK)
		_err = dialog_node.connect("portrait_changed", Callable(self, "_on_portrait_changed"))
		assert(_err == OK)
